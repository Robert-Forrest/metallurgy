...
def calculate_viscosity(alloy):
    ...
    sum_aG = 0
    for element in alloy.elements:
        sum_aG += mg.periodic_table.dict[element]['melting_temperature'] * alloy.composition[element] * np.log(
            (elementalViscosity[element] * (mg.periodic_table.dict[element]['mass'] / 1000)) / (plankConstant * avogadroNumber * (mg.periodic_table.dict[element]['density']) * 1000))
    ...
    viscosity = ((plankConstant * avogadroNumber) / (averageMolarVolume)) * np.exp((sum_aG - 0.155 * enthalpy.calculate_mixing_enthalpy(alloy)) /
                                                                                   (idealGasConstant * linear_mixture(alloy, 'melting_temperature')))
